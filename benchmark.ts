...
const runBenchmark = async (n: number, runs = 5) => {
  ...
  for (let i = 0; i < runs; i++) {
    ...
    try {
      ...
      console.log(`  Run ${i + 1}: Recursive (memoized) - ${(end - start).toFixed(2)}ms - Result: ${result}`);
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.log(`  Run ${i + 1}: Recursive (memoized) - Failed: ${errorMessage}`);
    }
  ...
  for (let i = 0; i < runs; i++) {
    ...
    try {
      ...
      console.log(`  Run ${i + 1}: Iterative - ${(end - start).toFixed(2)}ms - Result: ${result}`);
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  if (recursiveResults.length === 0) {
    ...
    const improvement = ((iterativeAvg - recursiveAvg) / iterativeAvg * 100).toFixed(2);
    console.log(`  🏆 Winner: Recursive (${improvement}% faster)`);
  } else if (iterativeAvg < recursiveAvg) {
    const improvement = ((recursiveAvg - iterativeAvg) / recursiveAvg * 100).toFixed(2);
